...
const getStatusBadge = (status) => {
    ...
    return (
        <Badge className={`${colors[status] || "bg-gray-500"} text-white text-xs px-2 py-0.5`}>
            {status?.replace(/_/g, " ")}
        </Badge>
    );
};
...
const ManagerProjectDetailContent = () => {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            const serviceKey = clientId && freelancerId
    ...
    const derivedTasks = useMemo(() => {
        return activeSOP.tasks.map((task) => {
            ...
            if (phaseStatus === "completed") return { ...task, uniqueKey, status: "completed", phaseName: taskPhase?.name };
            if (phaseStatus === "in-progress" && task.status === "completed") return { ...task, uniqueKey, phaseName: taskPhase?.name };
            return { ...task, uniqueKey, status: phaseStatus === "in-progress" ? "in-progress" : "pending", phaseName: taskPhase?.name };
        });
                ...
                <div className="w-full max-w-full mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <Card className="border border-border/60 bg-card/80 shadow-sm backdrop-blur">
                            ...
                            <CardContent>
                                <div className="text-2xl font-semibold">{Math.round(overallProgress)}%</div>
                        ...
                        <Card className="border border-border/60 bg-card/80 shadow-sm backdrop-blur">
                            ...
                            <CardContent>
                                <div className="text-2xl font-semibold">{completedPhases}/{activeSOP.phases.length}</div>
                        ...
                        <Card className="border border-border/60 bg-card/80 shadow-sm backdrop-blur">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">Client Budget</CardTitle>
                                <DollarSign className="h-4 w-4 text-muted-foreground" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-semibold">{formatCurrency(project?.budget)}</div>
                                <p className="text-xs text-muted-foreground mt-2">
                                    Freelancer: <span className="text-emerald-500">{formatCurrency(freelancerPay)}</span>
                                </p>
                            </CardContent>
                        </Card>

                        <Card className={`border shadow-sm backdrop-blur ${disputeCount > 0 ? 'border-red-500/50 bg-red-500/5' : 'border-border/60 bg-card/80'}`}>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-sm font-medium">Disputes</CardTitle>
                                <AlertTriangle className={`h-4 w-4 ${disputeCount > 0 ? 'text-red-500' : 'text-muted-foreground'}`} />
                            </CardHeader>
                            <CardContent>
                                <div className={`text-2xl font-semibold ${disputeCount > 0 ? 'text-red-500' : ''}`}>{disputeCount}</div>
                                <p className="text-xs text-muted-foreground mt-2">{disputeCount > 0 ? 'active' : 'no issues'}</p>
                            </CardContent>
                        </Card>
                    </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <Card className="border border-border/60 bg-card/80 shadow-sm backdrop-blur">
                                <CardHeader className="pb-3">
                                    ...
                                    <CardDescription>Monitor each phase of the project</CardDescription>
                                </CardHeader>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <Badge
                                                        ...
                                                        className={phase.status === "completed" ? "bg-emerald-500 text-white" : ""}
                                                    >
                                                ...
                                                <AccordionTrigger className="hover:no-underline py-3">
                                                    <div className="flex items-center gap-3 flex-1">
                                                        ...
                                                        <div className="flex-1 text-left">
                                                            <div className="font-semibold text-sm">{phaseGroup.phaseName}</div>
                        ...
                        <div className="space-y-6">
                            ...
                            <Card className="flex flex-col h-96 border border-border/60 bg-card/80 shadow-sm backdrop-blur">
                                <CardHeader className="border-b border-border/60 pb-3">
                                    ...
                                    <CardDescription>Chat with Client & Freelancer</CardDescription>
                                </CardHeader>
                                    ...
                                    {messages.length === 0 ? (
                                        ...
                                        <>
                                            {messages.map((msg) => (
                                                ...
                                                >
                                                    <p className={`text-xs font-medium ${msg.senderRole === "PROJECT_MANAGER"
                                                            ? "text-primary"
                            ...
                            <Card className="border border-border/60 bg-card/80 shadow-sm backdrop-blur">
                                ...
                                <CardContent>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-medium">{project?.owner?.fullName || "Unknown"}</p>
